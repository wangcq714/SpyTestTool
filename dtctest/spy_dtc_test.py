...
from dtctest.modules.writefile import *
from dtctest.report.testreport import *


#生成SpyCCode.c函数
def SpyCCodemain(pathname):
	global_var.DTC_Datas = []
	global_var.Num_DTC = 0
	global_var.ChannalMapping = {}
	global_var.ChannalMappingConvert = {}
	#调用选择文件夹窗口
	...
	if pathname != ():
		...
		global_var.DiagCH = config.MessageTxCH2SpyCH[str(config.Can2num[global_var.ChannalMapping[DiagCH].lower()])]
...
#生成测试报告函数
def reportmain(pathname, RouterTablepathname):
	global_var.DTC_Datas = []
	global_var.Num_DTC = 0
	global_var.ChannalMapping = {}
	global_var.ChannalMappingConvert = {}
